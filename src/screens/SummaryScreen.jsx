...
// Date + heure courtes pour horodater une analyse IA (ex: « 20 juin, 14:30 »).
...
export default function SummaryScreen({ onGoSettings }) {
  ...
  // Construit un résumé texte des dépenses à envoyer à Gemini.
  function buildSummaryText() {
    const lines = [`Nombre de dépenses : ${expenses.length}`]
  ...
  return (
    <div className="min-h-full overflow-x-hidden">
      <header className="px-5 pt-6 pb-4">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-50">Résumé</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Tes totaux par devise, catégorie et mois.
        </p>
      </header>

      {empty ? (
        <p className="text-center text-gray-400 mt-16 px-6 dark:text-gray-500">
          Ajoute des dépenses pour voir apparaître ton résumé.
        </p>
      ) : (
        <div className="px-4 pb-8 space-y-6">
          ...
          {/* Analyse IA : observation + blague générées par Gemini */}
          <section className="min-w-0 bg-white rounded-2xl p-4 shadow-sm dark:bg-gray-900">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="font-semibold text-gray-800 dark:text-gray-100">Analyse IA</h3>
            </div>
            <p className="text-sm text-gray-500 mb-3 dark:text-gray-400">
              Gemini commente l'état de tes dépenses et te glisse une petite
              blague.
            </p>
            ...
            >
              {aiLoading ? (
                ...
                <>
                  <Sparkles size={18} />
                  {latestAnalysis ? 'Relancer une analyse' : 'Analyser mes dépenses'}
                </>
              )}
            </button>
            ...
            {latestAnalysis && (
              <div className="mt-3 rounded-2xl border border-indigo-100 bg-indigo-50/60 p-3.5 dark:border-indigo-900 dark:bg-indigo-950/40">
                {latestAnalysis.analyse && (
                  <p className="text-sm leading-relaxed text-gray-700 dark:text-gray-200">
                    {latestAnalysis.analyse}
                  </p>
                )}
                {latestAnalysis.blague && (
            ...
            {analyses.length > 1 && (
              <details className="mt-3">
                <summary className="cursor-pointer list-none text-sm font-medium text-indigo-600 select-none dark:text-indigo-400">
                  Analyses précédentes ({analyses.length - 1})
                </summary>
          ...
          {/* Par mois */}
          <section className="min-w-0 bg-white rounded-2xl p-4 shadow-sm dark:bg-gray-900">
            <h3 className="font-semibold text-gray-800 mb-3 dark:text-gray-100">Par mois</h3>
          ...
          >
            <Download size={20} />
            Exporter en CSV ({expenses.length})
          </button>
        </div>
      )}
    </div>
